...
from dataset.dataloader_2 import DataLoader
...
class Pipeline():
    ...
    def __init__(self, cfg) -> None:
        ...
        self.train_loader = DataLoader(cfg.train_loader, cfg.augment, split='train', test=cfg.test, epochs=cfg.epochs)
        if self.eval:
            self.val_loader = DataLoader(cfg.train_loader, cfg.augment, split='val')
        self.epochs = cfg.epochs
        self.loss = getattr(losses, cfg.model.loss.name)(self.train_loader.weights, **cfg.model.loss.params)
        ...
        self.optimizer = tfa.optimizers.AdamW(learning_rate=getattr(utils, 
                    cfg.optimizer.schedule.name)(**cfg.optimizer.schedule.params, iterations=len(self.train_loader)),
                                              weight_decay=1e-5, clipnorm=1.0)
        #self.optimizer = getattr(optimizers, cfg.optimizer.name)(**cfg.optimizer.schedule.params)
    ...
    def train(self, e):
        ...
        self.train_loader.create_seq_id()
        ...
        self.optimizer.weight_decay = self.optimizer.learning_rate.lr * WD_RATIO

        for i, batch in enumerate(self.train_loader):
            #if i < 520:
            #    continue
            X, X_aug, X_mask, y_true = batch
            mask = X_mask#X_mask != 0
            mask = tf.concat([ tf.gather(mask, indices=utils.LIP, axis=2), 
                            mask[:, :, 468:, :] ], axis=2)
            #X = self.norm_input(X)
            mask = mask[:, :, :, :X.shape[-1]]

            #X_aug = tf.concat([X, X_aug], 0)
            #y_true = tf.concat([y_true, y_true], 0)

            with tf.GradientTape() as tape:
                y_pred = self.model(X_aug, training=True)
                mse, cce = self.loss.calc_loss([X, y_true], y_pred, mask)
            ...
            if mse is None:
                ...
                cce_loss += cce.numpy()
                mse_loss+= mse.numpy().mean()
            self.acc.update_state(y_true, y_pred[1])
            ...
            print("mse: {:.4f} cce: {:.4f} iteration: {}/{} epoch: {}/{}, lr: {:.2E}".format(mse_loss/(i+1), cce_loss / (i + 1), i, 
                size, e, self.epochs, self.optimizer.lr.lr ), end='\r')
        print()
        if y_pred[1].shape.rank == 2:
            #acc_score = accuracy_score(true_signs, pred_signs)
            acc_score = self.acc.result().numpy()
